...
f1 = @(x1,x2) 8/9*x1-1/18*x2+0.01*random('unif',-1,1);
f2 = @(x1,x2) x2+x1;
    
% initial set
viscircles([0.5,1], 0.25,'Color','g')
% domain
viscircles([0,0], 4,'Color','g')
% unsafe
% y>=1.5 or y<=1.5
...
unsafe = 0;
for i = [1:100] 
    listx = [random('unif',0.25,0.75)];
    listy = [random('unif',0.75,1.25)];
    if 0<=listx&&listx<=0.5&&0<=listy&&listy<=1
        continue
    end
    total = total + 1;
    count = 0;
